...
export const CollectionBreadcrumb = (props: CollectionBreadcrumbProps) => {
  ...
  const breadcrumbLinks = useMemo(() => {
    ...
    ): { collection_id: number; name: string; hasSubcollection: boolean }[] => {
      let collectionData: { collection_id: number; name: string; hasSubcollection: boolean }[] = [];

      collections.forEach((collection) => {
        collectionData.push({
          collection_id: collection.collection_id,
          name: collection.name,
          hasSubcollection: collection.subcollection_count > 1
        });
