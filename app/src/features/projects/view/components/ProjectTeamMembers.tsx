...
const ProjectTeamMembers = () => {
  ...
  const projectTeamMembers: IProjectParticipantsRoles[] = projectData.participants.map((member) => {
  ...
  <Stack spacing={1}>
    {projectTeamMembers.map((member) => (
      <Grid container spacing={0} key={member.display_name} sx={{ justifyContent: 'space-between', display: 'flex' }}>
        ...
        <Grid item sm={4}>
          <Typography component="dd" color="textSecondary" variant="subtitle2" sx={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>
