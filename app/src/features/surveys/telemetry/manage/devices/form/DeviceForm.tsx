...
export const DeviceForm = (props: IDeviceFormProps) => {
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 3 }}>
      <Paper sx={{ p: 5 }}>
        <Stack gap={5}>
          <HorizontalSplitFormComponent title="Device Details" summary="Enter information about the device.">
            <DeviceDetailsForm
              deviceMakes={
                codesContext.codesDataLoader.data?.telemetry_device_makes?.map((data) => ({
                  label: data.name,
                  value: data.id
                })) ?? []
              }
