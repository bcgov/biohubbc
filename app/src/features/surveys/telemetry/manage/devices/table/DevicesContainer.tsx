...
export const DevicesContainer = () => {
  ...
  const deviceDataLoader = useDataLoader((projectId: number, surveyId: number) =>
    biohubApi.telemetryDevice.getDevicesInSurvey(projectId, surveyId)
  );

  useEffect(() => {
    deviceDataLoader.load(surveyContext.projectId, surveyContext.surveyId);
  }, [deviceDataLoader, surveyContext.projectId, surveyContext.surveyId]);

  const devices = deviceDataLoader.data?.devices ?? [];
  const devicesCount = deviceDataLoader.data?.count ?? 0;
        ...
        <LoadingGuard
          ...
          <Box p={2}>
            <LoadingGuard
              ...
              <DevicesTable devices={devices} selectedRows={selectedRows} setSelectedRows={setSelectedRows} />
            </LoadingGuard>
          </Box>
        </LoadingGuard>
      </Box>
