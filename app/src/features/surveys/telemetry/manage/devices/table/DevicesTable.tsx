...
export const DevicesTable = (props: IDevicesTableProps) => {
  const { devices, selectedRows, setSelectedRows } = props;
  ...
  const handleDeleteSamplingSite = async () => {
    ...
    await biohubApi.samplingSite
      .deleteSampleSite(surveyContext.projectId, surveyContext.surveyId, actionMenuDeviceId)
      .then(() => {
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Box sx={{ display: 'flex', flexWrap: 'wrap' }}>
          {codesContext.codesDataLoader.data?.device_make.find(
