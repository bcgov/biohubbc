...
interface SurveyChecklistContainerProps {
 ...
 *
 * @param {SurveyChecklistContainerProps} props
 * @returns
 */
export const SurveyChecklistContainer = (props: SurveyChecklistContainerProps) => {
  const { checklist, activeView, handleViewChange, handleCheckboxClick } = props;

  const { surveyId } = useSurveyContext();

  // Prefix survey id to make the expanded state specific to each survey
