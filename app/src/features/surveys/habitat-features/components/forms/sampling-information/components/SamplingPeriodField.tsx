...
export const SamplingPeriodField = (props: ISamplingPeriodFieldProps) => {
  ...
  const [currentOption, setCurrentOption] = useState<SamplingInformationCachedPeriod | null>(
    values.survey_sample_period_id ? samplingInformationCache.getCurrentPeriod(values.survey_sample_period_id) : null
  );
  const [options, setOptions] = useState<SamplingInformationCachedPeriod[]>(
    samplingInformationCache.getPeriodsForRow(values.method_technique_id ?? null)
  );
  ...
  const getOptions = useMemo(
    () =>
      debounce(async (_searchTerm: string) => {
        ...
        const methodTechniqueId = values.method_technique_id;
        ...
        const validOptions = samplingInformationCache.getPeriodsForRow(values.method_technique_id ?? null);
    ...
    [
      ...
      values.survey_sample_site_id,
      values.method_technique_id,
      isMounted,
      samplingInformationCache,
  ...
  useEffect(() => {
    if (!values.survey_sample_site_id || !values.method_technique_id) {
    ...
    if (
      currentOption?.survey_sample_site_id !== values.survey_sample_site_id ||
      currentOption?.method_technique_id !== values.method_technique_id
    ) {
      ...
      setOptions(samplingInformationCache.getPeriodsForRow(values.method_technique_id));
