...
const SelectedSurveySpecies = (props: ISelectedSpeciesProps) => {
  const { name, selectedSpecies, speciesIndex, handleRemoveSpecies } = props;
  ...
  useEffect(() => {
    ecologicalUnitDataLoader.load(selectedSpecies.tsn);
  });

  const ecologicalUnitsForSpecies = ecologicalUnitDataLoader.data ?? [];

  const selectedUnits =
    values.species.focal_species
      .filter((species) => species.tsn === selectedSpecies.tsn)
      .flatMap((species) => species.ecological_units) ?? [];

  return (
    <Paper component={Stack} gap={3} variant="outlined" sx={{ p: 3, background: grey[50], my: 1 }}>
      <SpeciesSelectedCard index={speciesIndex} species={selectedSpecies} handleRemove={handleRemoveSpecies} />
          ...
          <Stack gap={2}>
            ...
            <Box>
              <Button
                ...
                disabled={
                  !(ecologicalUnitsForSpecies.length && selectedUnits.length < ecologicalUnitsForSpecies.length)
                }
