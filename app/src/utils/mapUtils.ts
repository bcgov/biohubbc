...
const _generateCustomHabitatFeatureMarkerUrl = (color?: string) =>
  'data:image/svg+xml;base64,' +
  btoa(
    `<svg width="24px" height="24px" viewBox="-3 0 24 24" xmlns="http://www.w3.org/2000/svg"><path fill="${
      color ?? SURVEY_MAP_LAYER_COLOURS.DEFAULT_COLOUR
...
/**
 * Returns custom map marker for symbolizing habitat feature points.
 *
 * @param {ColoredCustomMarkerProps} props
 * @return {*}  {L.Marker<any>}
 */
export const coloredCustomHabitatFeatureMarker = (props: ColoredCustomMarkerProps): L.Marker<any> => {
  return new L.Marker(props.latlng, {
    icon: L.icon({
      iconUrl: _generateCustomHabitatFeatureMarkerUrl(props.fillColor),
      iconSize: [20, 15],
      iconAnchor: [12, 12]
    })
  });
