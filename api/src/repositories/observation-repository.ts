...
export class ObservationRepository extends BaseRepository {
   ...
   * @param {Knex.QueryBuilder} userSurveyIds The query builder for retrieving survey IDs.
   * @return {Knex.QueryBuilder} The base query for retrieving survey observations.
   */
  _getSurveyObservationsBaseQuery(knex: Knex, userSurveyIds: Knex.QueryBuilder): Knex.QueryBuilder {
    return knex
      .with(
        'w_survey_sample_site',
        knex
   ...
   */
  _applyPagination(query: Knex.QueryBuilder, pagination?: ApiPaginationOptions): Knex.QueryBuilder {
    if (pagination) {
      query.limit(pagination.limit).offset((pagination.page - 1) * pagination.limit);
   ...
   */
  private _makeObservationListQuery(
    isUserAdmin: boolean,
    systemUserId: number | null,
    filterFields: IObservationAdvancedFilters
  ): Knex.QueryBuilder {
    ...
    // if user is admin, return all projects, otherwise join tables to determine user's projects
    if (!isUserAdmin) {
      userSurveyIds.whereIn('p.project_id', (subqueryBuilder) =>
        subqueryBuilder
          .select('p.project_id')
          .from('project as p')
          .leftJoin('project_participation as ppa', 'ppa.project_id', 'p.project_id')
          .where('ppa.system_user_id', systemUserId)
      );
    }

    const query = this._getSurveyObservationsBaseQuery(knex, userSurveyIds);
  ...
  async getSurveyObservationsWithSamplingDataWithAttributesData(
    ...
    const baseQuery = this._getSurveyObservationsBaseQuery(
      knex,
      knex.select('survey_id').from('survey').where('survey_id', surveyId)
    );

    if (pagination) {
      this._applyPagination(baseQuery, pagination);
    }
