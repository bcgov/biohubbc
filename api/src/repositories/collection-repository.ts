  ...
  _getCollectionsFlatQuery(queryBuilder: Knex.QueryBuilder): Knex.QueryBuilder {
    ...
    return queryBuilder
      .select(
        ...
        knex.raw('0::integer AS subcollection_count')
      )
      .from('collection')
        ...
        qb.select(
          ...
          .unionAll(function () {
            this.select(
              ...
              knex.raw(`
                COALESCE(
                  COUNT(nh.collection_id)::integer,
                  0
                ) AS subcollection_count
              `),
              knex.raw(`
                jsonb_build_array(
                  jsonb_build_object(
                    ...
                    'subcollection_count', nh.subcollection_count
                  )
                ) AS subcollections
              `)
            )
    ...
    query
      ...
      .with('final_collection_structure', (qb) => {
        qb.select(
          ...
          knex.raw(`
            COALESCE(
              COUNT(child.collection_id)::integer,
              0
            ) AS subcollection_count
          `),
          knex.raw(`
          COALESCE(
            jsonb_agg(
              jsonb_build_object(
                ...
                'subcollection_count', child.subcollection_count
              )
      ...
      .select(
        ...
        'collection.subcollection_count'
      )
