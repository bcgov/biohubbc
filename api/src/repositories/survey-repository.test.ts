...
describe('SurveyRepository', () => {
  ...
  describe('getBioHubSubmissionId', () => {
    it('should return the biohub_submission_id when one is set', async () => {
      const mockResponse = { rows: [{ biohub_submission_id: 42 }], rowCount: 1 } as any as Promise<QueryResult<any>>;
      const dbConnection = getMockDBConnection({ sql: () => mockResponse });

      const repository = new SurveyRepository(dbConnection);
      const result = await repository.getBioHubSubmissionId(1);

      expect(result).to.equal(42);
    });

    it('should return null when biohub_submission_id is not set', async () => {
      const mockResponse = { rows: [{ biohub_submission_id: null }], rowCount: 1 } as any as Promise<QueryResult<any>>;
      const dbConnection = getMockDBConnection({ sql: () => mockResponse });
    ...
    it('should throw ApiExecuteSQLError when no rows are returned', async () => {
      const mockResponse = { rows: [], rowCount: 0 } as any as Promise<QueryResult<any>>;
      const dbConnection = getMockDBConnection({ sql: () => mockResponse });
      ...
      try {
        await repository.getBioHubSubmissionId(1);
        expect.fail('Should have thrown an error');
      } catch (error) {
        expect((error as Error).message).to.equal('Failed to get BioHub submission ID for survey');
      }
    });
  });

  describe('setBioHubSubmissionId', () => {
    it('should set the biohub_submission_id without error', async () => {
      const mockResponse = { rows: [], rowCount: 1 } as any as Promise<QueryResult<any>>;
      ...
      await repository.setBioHubSubmissionId(1, 42);
