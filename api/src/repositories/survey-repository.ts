...
export class SurveyRepository extends BaseRepository {
   ...
   */
  async deleteSurvey(p_survey_id: number): Promise<void> {
    const sqlStatement = SQL`
      DO $$

        DECLARE
          v_survey_id INTEGER := 1;
        ...
        BEGIN
          ...
          WHERE occurrence_submission_id IN (
            SELECT occurrence_submission_id FROM occurrence_submission WHERE survey_id = v_survey_id
          );
          ...
          WHERE survey_summary_submission_id IN (
            SELECT survey_summary_submission_id FROM survey_summary_submission WHERE survey_id = v_survey_id
          );
          ...
          WHERE survey_attachment_id IN (
            SELECT survey_attachment_id FROM survey_attachment WHERE survey_id = v_survey_id
          );
          ...
          WHERE survey_report_attachment_id IN (
            SELECT survey_report_attachment_id FROM survey_report_attachment WHERE survey_id = v_survey_id
          );
          ...
          WHERE critter_id IN (SELECT critter_id FROM critter WHERE survey_id = v_survey_id);
