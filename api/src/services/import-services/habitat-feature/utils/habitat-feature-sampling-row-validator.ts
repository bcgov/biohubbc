...
interface IRowValidatorParams {
  ...
  /**
   * CSV Config Utils for the habitat feature CSV.
   *
   * @type {CSVConfigUtils<HabitatFeatureCSVStaticHeader>}
   */
  utils: CSVConfigUtils<HabitatFeatureCSVStaticHeader>;
  /**
   * Optional sample period id to automatically associate the habitat feature row with.
   *
...
/**
 * Habitat feature Sampling Information Row Validator - This function will validate a row of habitat feature data and ensure
 * that the sampling information provided in the row matches a valid sampling period from the provided list of
 * sampling periods.
 *
 *
 * Successfull paths:
 *  1. No sampling information provided, but habitat feature date, latitude and longitude is provided
 *  2. Exact period match found using site, technique, and period
 *  3. Multiple periods match the site, technique, and period, but only one matches the habitat feature date and time
 *
 * @param {IRowValidatorParams} rowValidatorParams
 * @return {*} {CSVRowValidator}
 */
export function getHabitatFeatureSamplingInformationRowValidator(
  ...
  return (params) => {
    ...
    // Extract habitat feature date and time from the row
    const worksheetObservedDate = utils.getCellValue('OBSERVED_DATE', params.row) as string | null;
    const worksheetObservedTime = utils.getCellValue('OBSERVED_TIME', params.row) as string | null;
    ...
    if (samplePeriodId) {
      ...
      throw new ApiGeneralError('Invalid sample period id provided', [
        'habitat feature-sampling-row-validator->getHabitatFeatureSamplingInformationRowValidator',
        { survey_sample_period_id: samplePeriodId }
      ]);
    }

    // VALID: No sampling information provided, but habitat feature date / time is provided with lat / lon
    if (!worksheetHasSamplingInformation && worksheetObservedDate && worksheetLatitude && worksheetLongitude) {
      return [];
    }

    // Validate the habitat feature date, latitude, and longitude exist when sampling information is NOT provided
    if (!worksheetHasSamplingInformation) {
      const errors = validateWorksheetHasLatitudeLongitudeAndDate(
        worksheetObservedDate,
        worksheetLatitude,
        worksheetLongitude,
        utils,
        params
      ...
      // INVALID: Habitat feature date, latitude, and longitude must all be provided when sampling information is NOT provided
      if (errors.length) {
        return errors;
      }
    }
    ...
    //  >1: Multiple periods match the site, technique, and period (Filter by habitat feature date/time)
    ...
    // Filter the matching periods by the habitat feature date and time
    //
    // Rules:
    //  0 Matches: No matching periods found for habitat feature date/time (ERROR)
    //  1 Match: Exact period match found using habitat feature date and time (SUCCESS)
    //  >1 Match: Multiple periods match the habitat feature date and time (ERROR)
    return validateSinglePeriodMatchesWithObservedDateTime(
      worksheetObservedDate,
      worksheetObservedTime,
 ...
 *
 * @param {string | null} worksheetObservedDate The habitat feature date from the worksheet
 ...
 * @param {CSVConfigUtils<HabitatFeatureCSVStaticHeader>} utils The CSV Config Utils for the habitat feature CSV
...
export function validateWorksheetHasLatitudeLongitudeAndDate(
  worksheetObservedDate: string | null,
  ...
  // INVALID: Habitat feature date is required when sampling information is not provided
  if (!worksheetObservedDate) {
    errors.push({
      error: 'Habitat feature date is required when sampling information is not provided',
      solution: 'Please provide sampling information or an habitat feature date and time',
      header: utils.getWorksheetHeader('OBSERVED_DATE', params.row),
      cell: null
    });
  }
...
/**
 * Validate Single Period Matches with Habitat feature Date Time - This function will validate a list of sampling periods
 * (filtered by site, technique, and period) and ensure that the habitat feature date and time provided in the row matches
 * a valid sampling period from the list of sampling periods.
 *
 * @param {string | null} worksheetObservedDate The habitat feature date from the worksheet
 * @param {string | null} worksheetObservedTime The habitat feature time from the worksheet
 * @param {SurveySamplePeriodDetails[]} samplePeriods The list of sampling periods to validate against
 * @param {CSVConfigUtils<HabitatFeatureCSVStaticHeader>} utils The CSV Config Utils for the habitat feature CSV
 * @param {CSVRowParams} params The CSV Row Params
 * @return {*} {CSVRowError[]} A list of CSV row errors
 */
export function validateSinglePeriodMatchesWithObservedDateTime(
  worksheetObservedDate: string | null,
  worksheetObservedTime: string | null,
  samplePeriods: SurveySamplePeriodDetails[],
  utils: CSVConfigUtils<HabitatFeatureCSVStaticHeader>,
  params: CSVRowParams
) {
  const matchingPeriodsByObservedDateTime = findMatchingPeriodsWithObservedDateTime(
    worksheetObservedDate,
    worksheetObservedTime,
    samplePeriods
  );

  // VALID: Exact period match found using sampling information and habitat feature date and time
  if (matchingPeriodsByObservedDateTime.length === 1) {
    updateCSVRowState(params.row, {
      sample_period_id: matchingPeriodsByObservedDateTime[0].survey_sample_period_id
    });

    return [];
  }

  // INVALID: Unable to match the habitat feature date/time to any existing period uniquely
  if (matchingPeriodsByObservedDateTime.length === 0) {
    return [
      {
        error: 'Sampling period is ambiguous, unable to uniquely identify period using habitat feature date',
        solution:
          'Use an habitat feature date that falls within a single period start and end date, or explicitly add a period',
        header: utils.getWorksheetHeader('OBSERVED_DATE', params.row),
        cell: worksheetObservedDate
      },
      {
        error: 'Sampling period is ambiguous, unable to unquely identify period using habitat feature date and time',
        solution:
          'Use an habitat feature date and time that falls within a single period start and end date, or explicitly add a period',
        header: utils.getWorksheetHeader('OBSERVED_TIME', params.row),
        cell: worksheetObservedTime
      }
    ];
  }

  // INVALID: Multiple periods match the habitat feature date and time
  return [
    {
      error: 'More than one period matches the habitat feature date and time',
      solution:
        'Use a habitat feature date and time that falls within the period start and end date of a single period',
      header: utils.getWorksheetHeader('OBSERVED_DATE', params.row),
      cell: worksheetObservedDate
    },
    {
...
/**
 * Find Matching Periods with Habitat feature Date Time - This function will take an habitat feature date and time and find all
 * matching sampling periods from the provided samplePeriods.
 *
 * @param {(string | null)} observedDate
 * @param {(string | null)} observedTime
 * @param {SurveySamplePeriodDetails[]} samplePeriods
 * @return {*}
 */
export function findMatchingPeriodsWithObservedDateTime(
  observedDate: string | null,
  observedTime: string | null,
  samplePeriods: SurveySamplePeriodDetails[]
) {
  if (!observedDate) {
    // If no habitat feature date is provided, then no periods can be matched
    return [];
  }
  ...
  const observedDateTime = newDayjs(observedDate, observedTime);

  const matchingHabitatFeatureSamplePeriods = samplePeriods.filter((samplePeriod) => {
    ...
    // The habitat feature date time must be within the start and end date time of the sampling period
    return (
      observedDateTime.isSameOrAfter(samplePeriodStartDateTime) &&
      observedDateTime.isSameOrBefore(samplePeriodEndDateTime)
    );
  });

  return matchingHabitatFeatureSamplePeriods;
}
