...
describe('TelemetryVectronicService', () => {
  ...
  describe('getVectronicBaseURL', () => {
    it('should return the default Vectronic API host', () => {
      const service = new TelemetryVectronicService(getMockDBConnection());

      const url = service.getVectronicBaseURL();

      expect(url.toString()).to.equal('https://api.vectronic-wildlife.com/');
    });
  });

  describe('getVectronicTelemetryURL', () => {
    it('should return the Vectronic API URL for the given query', () => {
      const service = new TelemetryVectronicService(getMockDBConnection());

      const url = service.getVectronicTelemetryURL({
        idcollar: 1,
        collarkey: 'test-collar-key',
        beforeAcquisition: '2021-01-01',
        afterAcquisition: '2021-02-01',
        gtId: 123
      });

      expect(url.toString()).to.equal(
        'https://api.vectronic-wildlife.com/?collarkey=test-collar-key&beforeAcquisition=2021-01-01&afterAcquisition=2021-02-01&gt-id=123'
      );
    });
    ...
    describe('fetchTelemetryFromVectronic', () => {
      it('should fetch telemetry data from the Vectronic API', async () => {
        ...
        expect(axiosStub).to.have.been.calledOnceWithExactly(
          'https://api.vectronic-wildlife.com/v2/collar/1/gps?collarkey=test-collar-key'
        );
    ...
    describe('fetchTelemetryCountFromVectronic', () => {
      it('should fetch telemetry count from the Vectronic API', async () => {
        ...
        expect(axiosStub).to.have.been.calledOnceWithExactly(
          'https://api.vectronic-wildlife.com/v2/collar/1/gps/count?collarkey=test-collar-key'
        );
