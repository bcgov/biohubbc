...
describe('xlsx/transform', () => {
  ...
  describe('transform file', () => {
    ...
    it('throws an error when req.body.occurrence_submission_id is empty', async () => {
      ...
      const requestHandler = transform.transform();
    ...
    it('returns a 200 if req.body.occurrence_submission_id exists', async () => {
      ...
      const transformFileStub = sinon.stub(ValidationService.prototype, 'transformFile').resolves();

      const requestHandler = transform.transform();
      await requestHandler(mockReq, mockRes, mockNext);
      expect(mockRes.statusValue).to.equal(200);
      expect(transformFileStub).to.have.been.calledOnceWith(mockReq.body.occurrence_submission_id);
      expect(mockRes.jsonValue).to.eql({ status: 'success' });
    });

    it('catches an error on transformFile', async () => {
      const dbConnectionObj = getMockDBConnection({ rollback: sinon.stub(), release: sinon.stub() });
      sinon.stub(db, 'getDBConnection').returns(dbConnectionObj);

      const transformFileStub = sinon
        .stub(ValidationService.prototype, 'transformFile')
        .throws(new Error('test transformFile error'));
      ...
      try {
        ...
        expect(transformFileStub).to.have.been.calledOnce;
        expect(errorServiceStub).to.have.been.calledOnce;
        expect(dbConnectionObj.rollback).to.have.been.calledOnce;
        expect(dbConnectionObj.release).to.have.been.calledOnce;
        expect((actualError as Error).message).to.equal('test transformFile error');
      }
    });
