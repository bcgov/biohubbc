...
import { CSVWorkBook, CSVWorksheet, ICsvState, XLSXCSV } from '../../utils/media/csv/csv-file';
import { getSPITemplateTransformers } from '../../utils/media/csv/spi/spi-transform';
...
export const POST: Operation = [
  logRequest('paths/spi/transform', 'POST'),
  prepSPITemplate(),
  validateSPITemplate(),
  transformSPITemplateToDWC()
];

POST.apiDoc = {
  description: 'Validate and transform a SPI template into Darwin Core.',
  tags: ['dwc'],
  security: [
    {
  ...
  requestBody: {
    description: 'SPI excel template file(s) to validate and transform into Darwin Core.',
    content: {
      'multipart/form-data': {
        schema: {
          type: 'object',
...
/**
 * Prepare a raw SPI template for valdiation/transformation.
...
function prepSPITemplate(): RequestHandler {
  return async (req, res, next) => {
    ...
    try {
      ...
      defaultLog.debug({ label: 'validateSPI', message: 'error', error });
      throw error;
    }
  };
}
...
function validateSPITemplate(): RequestHandler {
  return async (req, res, next) => {
    defaultLog.debug({ label: 'validateSPI', message: 'validateSPI' });
    ...
    try {
      ...
      if (hasErrors(validationResults)) {
        // found errors in one or more excel worksheets, return immediately
...
/**
 * Transform a SPI excel workbook into Darwin Core.
 *
 * @returns {RequestHandler}
 */
function transformSPITemplateToDWC(): RequestHandler {
  return async (req, res) => {
    defaultLog.debug({ label: 'transformSPI', message: 'transformSPI' });

    try {
      const xlsxCSV: XLSXCSV = req['xlsxCSV'];

      const targetWorkbook = new CSVWorkBook();

      // add a worksheet titled 'event' to the target workbook, and set the headers
      targetWorkbook.setWorksheet('event', new CSVWorksheet('event', xlsx.utils.aoa_to_sheet([['sex']])));

      xlsxCSV.transform(getSPITemplateTransformers(), targetWorkbook);

      console.log('=====================================');
      // console.log(targetWorkbook.worksheets.event);
      console.log('=====================================');

      // return res.status(200).json(xlsxCSV._workbook);
    } catch (error) {
      defaultLog.debug({ label: 'transformSPI', message: 'error', error });
      throw error;
    }
  };
}
