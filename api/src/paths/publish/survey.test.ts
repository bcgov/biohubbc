...
describe('survey', () => {
  ...
  describe('publishSurvey', () => {
    ...
    it('submits selected data to biohub', async () => {
      ...
      sinon.stub(PlatformService.prototype, 'submitSurveyDwcArchiveToBiohub').resolves({ uuid: 'test-uuid' });
    ...
    it('catches error, calls rollback, and re-throws error', async () => {
      ...
      sinon.stub(PlatformService.prototype, 'submitSurveyDwcArchiveToBiohub').rejects(new Error('a test error'));
