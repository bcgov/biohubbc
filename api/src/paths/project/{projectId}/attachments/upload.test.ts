...
describe('uploadMedia', () => {
  ...
  it('should succeed with valid params', async () => {
    ...
    const upsertProjectAttachmentStub = sinon
      .stub(AttachmentService.prototype, 'upsertProjectAttachment')
      .resolves({ project_attachment_id: 1, revision_count: 1, key: 'string' });

    const result = upload.uploadMedia();

    await result(mockReq, sampleRes as unknown as any, null as unknown as any);
    expect(actualResult).to.eql(expectedResponse);
    expect(upsertProjectAttachmentStub).to.be.calledOnce;
  });
});
