...
describe('getSingleTemplateURL', () => {
  ...
  it('should throw an error when projectId is missing', async () => {
    ...
    try {
      const result = get_signed_url.getSingleTemplateURL();
  ...
  it('should throw an error when templateId is missing', async () => {
    sinon.stub(db, 'getDBConnection').returns(dbConnectionObj);
    ...
    try {
      ...
      await result(
        { ...sampleReq, params: { ...sampleReq.params, templateId: null } },
        (null as unknown) as any,
        (null as unknown) as any
      );
      expect.fail();
      ...
      expect(actualError.status).to.equal(400);
      expect(actualError.message).to.equal('Missing required path param `templateId`');
    }
  });
  ...
  it('should throw a 400 error when no sql statement returned', async () => {
    ...
    sinon.stub(survey_occurrence_queries, 'getSurveyTemplateOccurrenceSQL').returns(null);
  ...
  it('should return null when getting signed url from S3 fails', async () => {
    ...
    sinon.stub(survey_occurrence_queries, 'getSurveyTemplateOccurrenceSQL').returns(SQL`some query`);
