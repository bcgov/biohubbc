...
describe('add a funding source', () => {
  ...
  it('should throw a 400 error when no request body present', async () => {
    ...
    try {
      ...
      expect((actualError as CustomError).status).to.equal(400);
      expect((actualError as CustomError).message).to.equal('Missing funding source data');
    }
  });
  ...
  it('should throw a 400 error when addFundingSource fails, because result has no rows', async () => {
    ...
    try {
      ...
      expect((actualError as CustomError).status).to.equal(400);
      expect((actualError as CustomError).message).to.equal('Failed to insert project funding source data');
    }
  });
  ...
  it('should throw a 400 error when no sql statement returned for addFundingSourceSQL', async () => {
    ...
    try {
      ...
      expect((actualError as CustomError).status).to.equal(400);
      expect((actualError as CustomError).message).to.equal('Failed to build addFundingSourceSQLStatement');
    }
  });
