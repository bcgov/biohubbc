...
_intended_outcome_name varchar := null;
...
begin
  with
    template_configuration1 as (
      select template_id from template where name = _template_name and version = _template_version and record_end_date is null
    ),
    field_method_configuration as (
      select field_method_id from field_method where name = _field_method_name and record_end_date is null
    ),
    intended_outcome_configuration as (
      select intended_outcome_id from intended_outcome where name = _intended_outcome_name and record_end_date is null
    ),
    template_configuration2 as (
      select template_methodology_species_id
        from
          template_methodology_species
        left join
          template_configuration1 on template_methodology_species.template_id = template_configuration1.template_id
        left join
          field_method_configuration on template_methodology_species.field_method_id = field_method_configuration.field_method_id
        left join
          intended_outcome_configuration on template_methodology_species.intended_outcome_id = intended_outcome_configuration.intended_outcome_id
        where
          template_methodology_species.template_id = template_configuration1.template_id
        and
          template_methodology_species.wldtaxonomic_units_id in (23919, 23921, 23922, 25929, 23920, 2064)
    )
  UPDATE
      template_methodology_species
    SET
    ...
    WHERE
      template_methodology_species_id IN (select template_methodology_species_id from template_configuration2);

  GET DIAGNOSTICS update_count = ROW_COUNT;
  RAISE NOTICE 'Number of rows updated: %', update_count ;
end
$$;
