...
------- delete above
    ...
    SELECT 
        ...
        msud.biohub_user_id,
        CASE 
            WHEN sss.role_type = 'Y' THEN 
                (SELECT survey_job_id FROM biohub.survey_job WHERE survey_job.name = 'Crew member')
            WHEN sss.role_type IN ('LEADER', 'COORDINATOR') THEN 
                (SELECT survey_job_id FROM biohub.survey_job WHERE survey_job.name = 'Crew lead')
            WHEN sss.role_type IN ('SURVEYOR', 'NAVIGATOR') THEN 
                (SELECT survey_job_id FROM biohub.survey_job WHERE survey_job.name = 'Biologist')
