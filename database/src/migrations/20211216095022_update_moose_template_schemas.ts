...
const moose_srb_or_composition_validation = fs.readFileSync(
  path.join(__dirname, VALIDATION_SCHEMAS_FOLDER, 'moose_srb_or_composition_survey_5.json')
);

const moose_srb_or_composition_transformation = fs.readFileSync(
  path.join(__dirname, TRANSFORMATION_SCHEMAS_FOLDER, 'moose_srb_or_composition_survey_5.json')
);
