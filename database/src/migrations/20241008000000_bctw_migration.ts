...
/**
 * Add new tables (migrated concepts from BCTW)
 * - device
 * - deployment (to replace the old `deployment` table which was renamed to `deployment_old`)
 * - telemetry_manual
...
export async function up(knex: Knex): Promise<void> {
  ...
  -- Add unique constraints
    ...
    COMMENT ON COLUMN deployment.deployment_id                  IS '(Generated) Surrogate primary key identifier.';
    ...
    COMMENT ON COLUMN deployment.device_key                      IS '(Generated) The SIMS unique key for the device.';
    COMMENT ON COLUMN deployment.frequency                       IS 'The frequency of the device.';
    COMMENT ON COLUMN deployment.frequency_unit_id               IS 'Foreign key to the frequency_unit table.';
    COMMENT ON COLUMN deployment.attachment_start_date           IS 'The date the device was attached to the animal.';
    COMMENT ON COLUMN deployment.attachment_start_time           IS 'The time the device was attached to the animal.';
    COMMENT ON COLUMN deployment.attachment_start_timestamp      IS '(Generated) The timestamp the device was attached to the animal.';
    COMMENT ON COLUMN deployment.attachment_end_date             IS 'The date the device was removed from the animal.';
    COMMENT ON COLUMN deployment.attachment_end_time             IS 'The time the device was removed from the animal.';
    COMMENT ON COLUMN deployment.attachment_end_timestamp        IS '(Generated) The timestamp the device was removed from the animal.';
    COMMENT ON COLUMN deployment.critterbase_start_capture_id    IS 'UUID of an external Critterbase capture record. The capture event during which the device was attached to the animal.';
    ...
    CREATE INDEX deployment_idx4 ON deployment(frequency_unit_id);

    -- Add indexes
    CREATE INDEX deployment_idx5 ON deployment(device_key);
    ----------------------------------------------------------------------------------------
    ...
    -- Add indexes for foreign keys
    CREATE INDEX telemetry_manual_idx1 ON deployment(deployment_id);
    ...
    ----------------------------------------------------------------------------------------
    CREATE TRIGGER audit_deployment BEFORE INSERT OR UPDATE OR DELETE ON biohub.deployment FOR EACH ROW EXECUTE PROCEDURE tr_audit_trigger();
    CREATE TRIGGER journal_deployment AFTER INSERT OR UPDATE OR DELETE ON biohub.deployment FOR EACH ROW EXECUTE PROCEDURE tr_journal_trigger();
