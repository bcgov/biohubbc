...
export async function up(knex: Knex): Promise<void> {
  await knex.raw(`
    ...
    COMMENT ON COLUMN collection.record_effective_date  IS 'Record level effective date.';
    ...
    CREATE TABLE collection_participation(
      ...
      collection_participation_role_id                    integer           NOT NULL,
      create_date                          timestamptz(6)    DEFAULT now() NOT NULL,
      create_user                          integer           NOT NULL,
      update_date                          timestamptz(6),
      update_user                          integer,
    ...
    COMMENT ON COLUMN collection_participation.collection_participation_role_id                   IS 'System generated surrogate primary key identifier.';
    ...
    CREATE TABLE collection_participation_role(
      collection_participation_role_id          integer           GENERATED ALWAYS AS IDENTITY (START WITH 1 INCREMENT BY 1),
      ...
      update_user               integer,
      revision_count            integer           DEFAULT 0 NOT NULL,
      CONSTRAINT collection_participation_role_pk PRIMARY KEY (collection_participation_role_id)
    );

    COMMENT ON COLUMN collection_participation_role.collection_participation_role_id         IS 'System generated surrogate primary key identifier.';
    COMMENT ON COLUMN collection_participation_role.name                     IS 'The name of the collection role.';
    COMMENT ON COLUMN collection_participation_role.description              IS 'The description of the collection role.';
    COMMENT ON COLUMN collection_participation_role.notes                    IS 'Notes associated with the record.';
    COMMENT ON COLUMN collection_participation_role.record_effective_date    IS 'Record level effective date.';
    COMMENT ON COLUMN collection_participation_role.record_end_date          IS 'Record level end date.';
    COMMENT ON COLUMN collection_participation_role.create_date              IS 'The datetime the record was created.';
    COMMENT ON COLUMN collection_participation_role.create_user              IS 'The id of the user who created the record as identified in the system user table.';
    COMMENT ON COLUMN collection_participation_role.update_date              IS 'The datetime the record was updated.';
    COMMENT ON COLUMN collection_participation_role.update_user              IS 'The id of the user who updated the record as identified in the system user table.';
    COMMENT ON COLUMN collection_participation_role.revision_count           IS 'Revision count used for concurrency control.';
    COMMENT ON TABLE  collection_participation_role                          IS 'collection roles.';
    ...
    ALTER TABLE collection_participation ADD CONSTRAINT collection_participation_fk3
      FOREIGN KEY (collection_participation_role_id)
      REFERENCES collection_participation_role(collection_participation_role_id);
    ...
    CREATE INDEX collection_participation_idx3 ON collection_participation(collection_participation_role_id);

    ----------------------------------------------------------------------------------------
    -- Create Indexes and Constraints for table: collection_participation_role
    ----------------------------------------------------------------------------------------

    -- Add unique end-date key constraint (don't allow 2 surveys with the same name and a NULL record_end_date)
    CREATE UNIQUE INDEX collection_participation_role_nuk1 ON collection_participation_role(name, (record_end_date is NULL)) where record_end_date is null;
    ...
    create trigger audit_collection_participation_role before insert or update or delete on collection_participation_role for each row execute procedure tr_audit_trigger();
    create trigger journal_collection_participation_role after insert or update or delete on collection_participation_role for each row execute procedure tr_journal_trigger();
