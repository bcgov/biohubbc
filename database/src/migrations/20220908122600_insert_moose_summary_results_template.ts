...
enum TEMPLATE_NAME {
  MOOSE_SUMMARY_RESULTS = 'Moose Summary Results_1.0'
}

enum COMMON_SURVEY_METHODOLOGY {
  DESCRIBED_IN_COMMENTS = 'Described in Comments'
}
...
const validationSchema = [
  ...
  {
    v_schema: JSON.stringify(mooseSummary),
    field_method: COMMON_SURVEY_METHODOLOGY.DESCRIBED_IN_COMMENTS,
    template: TEMPLATE_NAME.MOOSE_SUMMARY_RESULTS
  }
];
...
export async function up(knex: Knex): Promise<void> {
  await knex.raw(`
    ...
    values
        ('${TEMPLATE_NAME.MOOSE_SUMMARY_RESULTS}', '1.0', now(), '${TEMPLATE_NAME.MOOSE_SUMMARY_RESULTS}');
    `);
