...
/**
 * Update the project funding source trigger to reference the `agency` table (renamed from `funding_source`).
 *
 * @export
 * @param {Knex} knex
 * @return {*}  {Promise<void>}
...
export async function up(knex: Knex): Promise<void> {
  ...
  --                  2023-05-09  SIMSBIOHUB-148 rename funding_source table to agency
  -- *******************************************************************
  declare
    _project_id_optional agency.project_id_optional%type;
  begin
    ...
    if ((new.investment_action_category_id is not null) and (new.funding_source_project_id is null)) then    
      select project_id_optional into _project_id_optional from agency
        where agency_id = (select agency_id from investment_action_category where investment_action_category_id = new.investment_action_category_id);
