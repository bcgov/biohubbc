...
begin
  ...
  insert into project_attachment (project_id, file_name, title, key, file_size, file_type, security_review_timestamp) values (_project_id, 'test_filename.txt', 'test filename', 'projects/'||_project_id::text, 10000, 'video', now()) returning project_attachment_id into _project_attachment_id;  
  ...
  select count(1) into _count from project_attachment_persecution;
  assert _count = 1, 'FAIL project_attachment_persecution';
  select count(1) into _count from project_attachment_proprietary;
  assert _count = 1, 'FAIL project_attachment_proprietary';
  select count(1) into _count from project_report_persecution;
  assert _count = 1, 'FAIL project_report_persecution';
  select count(1) into _count from project_report_proprietary;
  assert _count = 1, 'FAIL project_report_proprietary';
  ...
  insert into survey_attachment (survey_id, file_name, title, key, file_size, file_type, security_review_timestamp) values (_survey_id, 'test_filename.txt', 'test filename', 'projects/'||_project_id::text||'/surveys/'||_survey_id::text, 10000, 'video', now()) returning survey_attachment_id into _survey_attachment_id;
  ...
  select count(1) into _count from survey_attachment_persecution;
  assert _count = 1, 'FAIL survey_attachment_persecution';
  select count(1) into _count from survey_attachment_proprietary;
  assert _count = 1, 'FAIL survey_attachment_proprietary';
  select count(1) into _count from survey_report_persecution;
  assert _count = 1, 'FAIL survey_report_persecution';
  select count(1) into _count from survey_report_proprietary;
  assert _count = 1, 'FAIL survey_report_proprietary';
...
--  raise notice 'survey status (survey_id, survey_id, survey_status):';
