...
begin
  ...
  select count(1) into _count from spatial_transform;
  assert _count = 1, 'FAIL spatial_transform';
  ...
  --insert into system_user_role (system_user_id, system_role_id) values (_system_user_id, (select system_role_id from system_role where name = 'System Administrator'));
  insert into submission_spatial_component (occurrence_submission_id, spatial_component) values (_occurrence_submission_id, '{}') returning submission_spatial_component_id into _submission_spatial_component_id;
  -- xrefs
  ...
  select count(1) into _count from spatial_transform_submission;
  assert _count = 1, 'FAIL spatial_transform_submission';
