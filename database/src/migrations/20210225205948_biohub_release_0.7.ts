...
export async function up(knex: Knex): Promise<void> {
  const create_spatial_extensions = fs.readFileSync(path.join(__dirname, 'release.0.7', 'create_spatial_extensions.psql'));
  const biohub_0_7 = fs.readFileSync(path.join(__dirname, 'release.0.7', 'biohub.0.7.sql'));
  const populate_user_identity_source = fs.readFileSync(path.join(__dirname, 'release.0.7', 'populate_user_identity_source.sql'));
  ...
  const tr_project_funding_source = fs.readFileSync(path.join(__dirname, 'release.0.7', 'tr_project_funding_source.sql'));

  const populate_first_nations = fs.readFileSync(path.join(__dirname, 'release.0.7', 'populate_first_nations.sql'));
  const populate_climate_change_initiatives = fs.readFileSync(path.join(__dirname, 'release.0.7', 'populate_climate_change_initiatives.sql'));
  const populate_management_action_type = fs.readFileSync(path.join(__dirname, 'release.0.7', 'populate_management_action_type.sql'));
  const populate_funding_source = fs.readFileSync(path.join(__dirname, 'release.0.7', 'populate_funding_source.sql'));
  const populate_investment_action_category = fs.readFileSync(path.join(__dirname, 'release.0.7', 'populate_investment_action_category.sql'));
  ...
  const populate_icun_classifications = fs.readFileSync(path.join(__dirname, 'release.0.7', 'populate_icun_classifications.sql'));
  ...
  await knex.raw(`
    ...
    set search_path = project_management, public;
    ...
    set search_path = project_management;
    ...
    grant usage on schema project_management to ${DB_USER_API};
    ...
    alter role ${DB_USER_API} set search_path to biohub_dapi_v1, project_management, public, topology;
