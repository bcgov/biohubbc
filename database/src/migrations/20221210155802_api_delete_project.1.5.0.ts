...
--                  2022-11-15  addition of security label associative tables
 ...
 declare
   ...
   begin
     ...
     delete from project_attachment_persecution where project_attachment_id in (select project_attachment_id from project_attachment where project_id = p_project_id);
     delete from project_attachment_proprietary where project_attachment_id in (select project_attachment_id from project_attachment where project_id = p_project_id);
     delete from project_report_persecution where project_report_attachment_id in (select project_report_attachment_id from project_report_attachment where project_id = p_project_id);
     delete from project_report_proprietary where project_report_attachment_id in (select project_report_attachment_id from project_report_attachment where project_id = p_project_id);
