...
begin
  ...
  insert into project_permit (p_id, number, type, sampling_conducted) values (__id, random(), 'permit type', 'Y');  
  ...
  assert __count = 1, 'FAIL insert webform_draft';

  delete from no_sample_permit;
  insert into no_sample_permit (number, type, coordinator_first_name, coordinator_last_name, coordinator_email_address, coordinator_agency_name) values (random(), 'permit type', 'first name', 'last name', 'nobody@nowhere.com', 'agency');
  select count(1) into __count from no_sample_permit;
  assert __count = 1, 'FAIL insert no_sample_permit';
  ...
  assert __count = 1, 'FAIL insert administrative_activity';

-- delete target project data
  call api_delete_project(__id);
  select count(1) into __count from project;
  assert __count = 0, 'FAIL delete project';
end
$$;
