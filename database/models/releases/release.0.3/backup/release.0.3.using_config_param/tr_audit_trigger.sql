-- tr_audit_trigger.sql
create or replace function tr_audit_trigger() returns trigger
language plpgsql
security definer
as
$$
declare
  v_user_id system_user.id%type;
  v_user_identity_source_id user_identity_source.id%type;
begin
  select coalesce(current_setting('biohub_api.user_id', true), null)::integer into v_user_id;  

  raise notice '%', user||'.user_id';
  if (v_user_id is null) THEN    
    select a.id into strict v_user_id from biohub.system_user a, biohub.user_identity_source b
      where a.uis_id = b.id
      and b.name = 'DATABASE';
      
    perform set_config(user||'.user_id', v_user_id::character, false);
  end if;

  if (TG_OP = 'INSERT') then
    new.create_user = v_user_id;
  elsif (TG_OP = 'UPDATE') then
    new.update_user = v_user_id;
    new.update_date = now();
    -- create audit fields are immutable
    new.create_user = old.create_user;
    new.create_date = old.create_date;
  end if;

  -- concurrency
  if (tg_op = 'UPDATE') then
    if (new.revision_count != old.revision_count) THEN
      raise exception 'CONCURRENCY_EXCEPTION';
    else
      new.revision_count = (old.revision_count + 1);
    end if;
  end if;
  
  if (tg_op = 'DELETE') then
    return OLD;
  else
    return NEW;
  end if;
exception
  when others THEN
    raise;
end;
$$;
