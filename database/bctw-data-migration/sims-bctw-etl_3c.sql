--------------------------------------------------------------------------------------------------------------
-- Populate matched_collar_deployment table
--
-- Notes
...
with w_matched_collar_deployment as (
    select
...
INSERT INTO bctw.matched_collar_deployment (
...
update 
    bctw.matched_collar_deployment 
set 
    internal_is_valid = true 
where 
    matched_collar_deployment_id in (
        select
            matched_collar_deployment_id
        from
            bctw.matched_collar_deployment
        WHERE (
...
where 
    matched_collar_deployment_id in (
        ...
        SELECT
            t1.matched_collar_deployment_id
        FROM
            w_data as t1
        WHERE EXISTS (
            SELECT
            ...
            FROM
                w_data AS t2
            WHERE
                t1.matched_collar_deployment_id <> t2.matched_collar_deployment_id and
...
RETURNING matched_collar_deployment_id;

--------------------------------------------------------------------------------------------------------------
-- Populate matched_collar and matched_deployment tables
--
-- Notes
...
WITH 
-- Get valid matched_collar_deployment rows
w_valid_matched_collar_deployment_rows AS (
    select
        *
    from
        bctw.matched_collar_deployment
...
w_resolved_valid_matched_collar_deployment_rows as (
    select
        matched_collar_deployment_id,
        --
        ...
        collar_animal_assignment_valid_to
    FROM
        w_valid_matched_collar_deployment_rows
    CROSS JOIN LATERAL (
        SELECT
        ...
        FROM 
            (
                ...
                FROM 
                    bctw.matched_collar_deployment ncd_inner
                WHERE 
                    ncd_inner.matched_collar_deployment_id = w_valid_matched_collar_deployment_rows.matched_collar_deployment_id
            ) AS unnested
    ) AS max_vals
),
-- Insert into matched_collar
w_matched_collar AS (
    INSERT INTO bctw.matched_collar (
        matched_collar_deployment_id,
        --
        bctw_collar_uuid,
        --
    ...
    select
        ...
        collar_updated_by
    FROM
        w_resolved_valid_matched_collar_deployment_rows
    RETURNING 
        *
)
-- Insert into matched_deployment
INSERT INTO bctw.matched_deployment (
    matched_collar_deployment_id,
    --
    bctw_deployment_uuid,
    --
...
select
    w_resolved_valid_matched_collar_deployment_rows.matched_collar_deployment_id,
    --
    w_resolved_valid_matched_collar_deployment_rows.bctw_deployment_uuid,
    --
    w_resolved_valid_matched_collar_deployment_rows.bctw_critter_uuid,
    --
    w_resolved_valid_matched_collar_deployment_rows.attachment_start,
    w_resolved_valid_matched_collar_deployment_rows.attachment_end,
    w_resolved_valid_matched_collar_deployment_rows.frequency,
    w_resolved_valid_matched_collar_deployment_rows.frequency_unit,
    --
    w_resolved_valid_matched_collar_deployment_rows.collar_animal_assignment_created_at,
    w_resolved_valid_matched_collar_deployment_rows.collar_animal_assignment_created_by,
    w_resolved_valid_matched_collar_deployment_rows.collar_animal_assignment_updated_at,
    w_resolved_valid_matched_collar_deployment_rows.collar_animal_assignment_updated_by
FROM
    w_matched_collar
JOIN
    w_resolved_valid_matched_collar_deployment_rows
ON
    w_resolved_valid_matched_collar_deployment_rows.matched_collar_deployment_id = w_matched_collar.matched_collar_deployment_id;